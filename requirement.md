...
4. 같은 뉴스는 한 번만 저장 (URL 기반 중복 제거)
5. AI가 뉴스 성격과 지표들을 분석 후 인사이트 정리 (Gemini 2.5 Flash 모델 사용)
6. 결과를 JSON 파일로 저장 → GitHub에 자동 커밋
7. 팀즈 채널에 알람 발송
8. 프론트엔드(GitHub Pages)는 JSON 파일을 fetch하여 표시
...
**8.5.2 주요 뉴스 헤드라인 (v2.2 개선)**

* 목적: 그날의 **진짜 중요한 뉴스**를 파악
* 선정 기준: **유사 기사 그룹핑 → 가장 많이 다뤄진 주제 순**
* 동작 방식:
  1. 전체 기사의 제목 유사도 계산 (Jaccard Similarity)
  2. 유사한 기사들을 그룹으로 묶음 (70% 이상 유사)
  3. 그룹 크기(기사 수) 순으로 정렬
  4. 각 그룹에서 대표 기사 1개 선정
  5. 상위 5개 헤드라인 표시
* 클릭 시 원문 이동
...
* 틀 없이 무역/물류/SCM 관점을 종합하여 3줄로 요약
